...
## 10. 从 GitHub 还原 checkpoint

GitHub 分支中的 15 个 `best.pt` 以 `.partNNN` 无损分块保存。克隆或下载本目录后运行：

```powershell
python reassemble_checkpoints.py
```

脚本会按 `checkpoint_parts_manifest.json` 重组全部 `best.pt`，并逐个核对文件大小与 SHA-256。原始分块会保留。
